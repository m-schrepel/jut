...
var drawPartTwo = function(canvasWidth, canvasHeight, numLines){
  ...
  // #1, make it so.
  for (key in data) {
    new Line().x1(gap).y1(barMax).x2(gap).y2(600-data[key]*difference)
    .stroke('cornflowerblue').strokeWidth(barWidth).draw(svg2);
    new Text().x(gap-textOffset).y(canvasHeight).text(key).fill('#333').fontSize('9').draw(svg2);
    gap = gap+23;
  };
  new Text().x(20).y(40).text('CO2 Emissions Monthly Avarages for 1959 - 1990').fontSize('14').fill('#333').draw(svg2);
